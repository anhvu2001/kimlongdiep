const idCategoryPost = document
  .querySelector(".mix-match-container-category-post")
  ?.getAttribute("data-id");
const pagination = document.getElementById("pagination-category-post");
const links = pagination.querySelectorAll("a");
butPrevBycategory = document.getElementById("btn-pagination-byCategory-prev");
butNextBycategory = document.getElementById("btn-pagination-byCategory-next");
...
const getMorePosts = async () => {
  ...
  const response = await fetch("https://kimlongdiep.com/graphql", {
    ...
    body: JSON.stringify({
      ...
      variables: {
        categoryId: parseInt(idCategoryPost),
        after: afterCursor,
        before: null,
        first: first,
        last: last,
...
function hideAndShowButton() {
  butPrevBycategory.style.display =
    hasPreviousPage || currentPage > 1 ? "flex" : "none";
    butNextBycategory.style.display = currentPage === totalPageCount ? "none" : "flex";
  butPrevBycategory.style.display = currentPage === 1 ? "none" : "flex";
}
getMorePosts();
hideAndShowButton();
if (links.length <= 1) {
...
function handlePrevClick() {
  ...
  currentPage === 1
    ? (butPrevBycategory.style.display = "none")
    : (butPrevBycategory.style.display = "flex");
}
butPrevBycategory.addEventListener("click", handlePrevClick);
butNextBycategory.addEventListener("click", handleNextClick);
showPageLinks(currentPage);
if (pagination) {
