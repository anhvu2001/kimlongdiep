...
class Theme_My_Login_Form
{
	...
	public function render_links()
	{
		...
		foreach ($links as $name => $link) {
			...
			$output .= sprintf(
				'<li class="tml-%s-link"><p>%s</p><a href="%s">%s</a></li>',
				esc_attr($name),
				$content, // Thay đổi phần này
				esc_url($link['url']),
				esc_html($link['text'])
			);
		}
	...
	{
		$output = '<form class="loginform" name="' . esc_attr($this->get_name()) . '" action="' . esc_url($this->get_action()) . '" method="' . esc_attr($this->get_method()) . '">';
		$output .= '<div class="tml-image-login"><img src="' . get_site_url() . '/wp-content/uploads/2023/05/imagelogin.png" alt="Image Kim Long Diệp"></div>';
		if ($this->get_name() === 'register') {
			$output .= '<div class="title-login-register"><h1>Đăng ký tài khoản</h1></div>';
		} else if($this->get_name() === 'login') {
			$output .= '<div class="title-login-register"><h1>Đăng nhập tài khoản</h1></div>';
		}
		else {
			$output .= '<div class="title-login-register"><h1>Lấy lại tài khoản</h1></div>';
		}
	...
	public function render($args = array())
	{
		...
		$output .= '<div class="tml-image-login tml-image-login-mobile"><img src="' . get_site_url() . '/wp-content/uploads/2023/05/imagelogin.png" alt="Image Kim Long Diệp"></div>';
