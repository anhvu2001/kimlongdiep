	...
	public function render( $args = array() ) {
		...
		switch ( $this->get_type() ) {
			...
			default:
				...
				$placeholder = '';
				// Tùy biến giá trị của placeholder dựa vào tên trường (name)
				switch ($this->get_name()) {
					...
					case 'pwd':
						...
						break;
					case 'user_login':
						if (strpos($current_url, '/lostpassword/') !== false) {
				...
				$output .= $error;
				$output .= $args['control_before'];
				$output .= '<input placeholder="' . $placeholder . '" name="' . $this->get_name() . '" type="' . $this->get_type() . '" value="' . esc_attr($this->get_value()) . '"' . $attributes . ">\n";
				$output .= $args['control_after'];
		}
