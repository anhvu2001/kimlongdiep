...
{ // Lấy danh sch cc danh mc của sn phẩm
	...
	?>
			<div class="title-product-child-detail">
				<h6 style="font-family: 'Montserrat'; font-style: normal; font-weight: 600;
				 font-size: 24px; line-height: 29px; display: flex; align-items: center; color: #FDCD81;">
					Sản phẩm <?php echo $category->name; ?> khác
				</h6>
				<hr style="border: none; border-top: 2px solid #FDCD81; width: 60%;">
...
function product_recently_viewed()
{
	...
	<div class="recented-product-detail-container">
		...
		foreach ($viewed_products as $product_id) {
			...
			if (!$product_image) {
				$product_image = wc_placeholder_img_src('full'); // Đặt ảnh mặc đnh nu không có nh
			}
		?>
			<div class="card-featured recented-product-detail-card">
				...
				<div class="recented-product-detail-card-info">
					...
					<p>
						<a class="recented-product-detail-card-price" href="<?php echo esc_url($product_permalink); ?>"><?php echo wp_kses_post(wc_price($product_price)); ?></a>
					</p>
				</div>
			</div>
		<?php
...
// đổi icon / thnh icon > trn thanh breadcumb trang chi tit sn phm
	...
	?>
		<script>
			document.addEventListener("DOMContentLoaded", function() {
				...
				// Gi hm setWidthBasedOnScreenSize khi trang đưc ti và thay ổi kích thc màn hình
				setWidthBasedOnScreenSize();
				window.addEventListener("resize", setWidthBasedOnScreenSize);
			});
		</script>
		<div class="bang-size-main">
			<h5>Cách đo Size <?php echo $categoryName; ?>:</h5>
			<a href="<?php echo get_site_url(); ?>/<?php echo $imagePath; ?>">
				<img class="bang-size-image" style="float: left; margin-right: 20px; width: 65%;" src="<?php echo get_site_url(); ?>/<?php echo $imagePath; ?>" alt="">
			</a>
...
//thay đi vị tr ca hình thc thanh ton
...
function get_short_description()
{
	...
	if ($product) {
		...
		if ($description) {
	?>
			<fieldset style="border: 1px solid #ffa61a; border-radius: 5px; margin-bottom: 20px;">
				<legend>Mô tả sản phẩm</legend>
				<p class="kld-text-description"><?php echo $description; ?></p>
				<style>
					.kld-text-description::first-letter {
...
function add_category_to_single_product($classes)
{
	if (is_product()) {
		global $post;
...
add_filter('body_class', 'add_category_to_single_product');

function hotjar_install_header_code() {
    ?>
...
add_action( 'wp_head', 'hotjar_install_header_code' );